    ...
    vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        for (int i = 0; i < segmentsnum; i++) {
            if ((i == 0 && heights[i] != 0) || (i != 0 && heights[i] != heights[i-1])) {
                // res[segments[i].begin].push_back(heights[i]);
                res.push_back(make_pair(segments[i].begin, heights[i]));
            }
        }
        if (heights[segmentsnum - 1] != 0) {
            // res[segments[segmentsnum - 1].begin].push_back(0);
            res.push_back(make_pair(segments[segmentsnum - 1].begin, 0));
        }
        return res;
    }
};
